...
def klasifikasiNada(k, windowLength, frameLength, mfccCoefficients, info, klasifikasiDum, klasifikasiTak, klasifikasiSlap, presentaseKlasifikasi):
  ...
  for i in range(20) :
    indeks = i + 51
    filename = 'D:/Ngoding/darbukaToneIdentification/static/dataset/toneBasic/dum/dum' + str(indeks) + '.wav'
  ...
  for i in range(20) :
    indeks = i + 51
    filename = 'D:/Ngoding/darbukaToneIdentification/static/dataset/toneBasic/tak/tak' + str(indeks) + '.wav'
    hasil, k_dum, k_tak, k_slap, indeks = classificationFunc.klasifikasi(filename, k, windowLength, frameLength, mfccCoefficients)
    file = filename.split(sep="/")
    file2 = file[len(file)-1]
    if hasil == 'TAK':
  ...
  for i in range(20) :
    indeks = i + 51
    filename = 'D:/Ngoding/darbukaToneIdentification/static/dataset/toneBasic/slap/slap' + str(indeks) + '.wav'
    hasil, k_dum, k_tak, k_slap, indeks = classificationFunc.klasifikasi(filename, k, windowLength, frameLength, mfccCoefficients)
    file = filename.split(sep="/")
    file2 = file[len(file)-1]
    if hasil == 'SLAP':
